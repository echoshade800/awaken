...
import AppleHealthKit, { getPermissionsConstants } from './modules/health/healthkitBridge';

// 获取权限配置
const Constants = getPermissionsConstants();
const healthKitPermissions = Constants.Permissions ? {
  permissions: {
    read: [
  ...
  return new Promise((resolve) => {
    AppleHealthKit.isAvailable((err, available) => {
      ...
      AppleHealthKit.getAuthStatus(healthKitPermissions, (authErr, results) => {
        ...
        if (stepsStatus === Constants.AuthorizationStatus?.SharingAuthorized ||
            stepsStatus === 2) {
          isHealthKitInitialized = true;
          resolve('granted');
        } else {
