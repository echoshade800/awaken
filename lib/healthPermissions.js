...
export async function checkStepPermission() {
  ...
  if (Platform.OS !== 'ios' || !AppleHealthKit) {
    console.log('[HealthPermissions] Non-iOS platform or HealthKit unavailable, returning denied');
    return 'denied';
  }
  ...
  return new Promise((resolve) => {
    AppleHealthKit.isAvailable((err, available) => {
      ...
      AppleHealthKit.getAuthStatus(healthKitPermissions, (authErr, results) => {
        if (authErr) {
...
export async function requestStepPermission() {
  ...
  return new Promise((resolve) => {
    AppleHealthKit.isAvailable((err, available) => {
      ...
      AppleHealthKit.initHealthKit(healthKitPermissions, (initErr) => {
        if (initErr) {
...
export async function fetchDailySteps14d() {
  console.log('[HealthPermissions] fetchDailySteps14d called, Platform:', Platform.OS);
  ...
  try {
    return new Promise((resolve, reject) => {
      ...
      AppleHealthKit.getDailyStepCountSamples(options, (err, results) => {
        ...
        console.log('[HealthKit] Fetched daily step samples:', results?.length || 0);
        ...
        console.log('[HealthKit] Formatted daily steps:', formattedData);
        resolve(formattedData);
      });
    });
  } catch (error) {
    console.error('[HealthPermissions] Error in fetchDailySteps14d:', error);
    return [];
  }
}
