...
const useStore = create((set, get) => ({
  ...
  initialize: async () => {
    try {
      ...
      // Migrate old alarms to include wakeMode, customDays, and ringtoneId if missing
      const migratedAlarms = alarms.map(alarm => {
        const needsMigration = !alarm.wakeMode || !alarm.customDays || !alarm.ringtoneId;
        if (needsMigration) {
          return {
            ...alarm,
            wakeMode: alarm.wakeMode || 'voice',
            customDays: alarm.customDays || [],
            ringtoneId: alarm.ringtoneId || 'lingling'
          };
        }
        return alarm;
      });
      ...
      const needsSave = migratedAlarms.some((a, i) => {
        const original = alarms[i];
        return a.wakeMode !== original?.wakeMode ||
               a.ringtoneId !== original?.ringtoneId ||
               JSON.stringify(a.customDays) !== JSON.stringify(original?.customDays);
      });
