...
const useStore = create((set, get) => ({
  ...
  personalizedSleepNeed: null,
  usingRealUsageData: false,
  ...
  enableUsageTracking: async () => {
    const hasPermission = await checkScreenTimePermission();
    const trackingStarted = await startUsageTracking();

    set({ usageTrackingEnabled: true, usingRealUsageData: hasPermission && trackingStarted });

    if (!hasPermission || !trackingStarted) {
      console.log('[Store] No Screen Time permission, generating demo data');
      await generateMockUsageData(30);
    } else {
      console.log('[Store] Screen Time permission granted, using real data');
    }

    const personalizedNeed = await calculateSleepNeedFromUsage(30);
    set({ personalizedSleepNeed: personalizedNeed });

    await get().refreshSleepCalculations();
  },

  disableUsageTracking: async () => {
    await stopUsageTracking();
    set({ usageTrackingEnabled: false, personalizedSleepNeed: null });
  },

  getEffectiveSleepNeed: () => {
    const { personalizedSleepNeed, sleepNeed } = get();
    return personalizedSleepNeed || sleepNeed;
  },
}));
