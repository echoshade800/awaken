  ...
  syncHealthKitData: async () => {
    try {
      ...
      if (!healthKitSessions || healthKitSessions.length === 0) {
        console.log('[Store] No HealthKit sleep data found');
        return { success: true, message: 'No data found', count: 0 };
      }

      console.log('[Store] Fetched HealthKit sessions:', healthKitSessions.length);
      ...
      const nonHealthKitSessions = nonDemoSessions.filter((s) => s.source !== 'healthkit');

      // Combine with new HealthKit data
      const allSessions = [...nonHealthKitSessions, ...healthKitSessions];
      ...
      console.log('[Store] HealthKit sync completed:', healthKitSessions.length, 'sessions');
      ...
      return {
        ...
        count: healthKitSessions.length,
      };
    } catch (error) {
