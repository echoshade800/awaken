...
import { bootstrapSleepFromHealthKit, inferSleepFromRecentSamples } from './sleepInference';
  ...
  syncHealthKitData: async () => {
    try {
      ...
      if (healthKitSessions && healthKitSessions.length > 0) {
        ...
        // Otherwise, try to infer from recent step samples (NEW METHOD)
        console.log('[Store] No direct sleep data, attempting step-based inference from 24h samples...');
        const inferenceResult = await inferSleepFromRecentSamples();

        if (inferenceResult.ok && inferenceResult.sessions.length > 0) {
          console.log('[Store] Successfully inferred sleep from step samples:', inferenceResult.sessions.length);
          sessionsToUse = inferenceResult.sessions;
          dataSource = 'healthkit-inferred';
        } else {
          console.log('[Store] inferenceResult had no usable sessions:', inferenceResult);
          return {
            success: false,
