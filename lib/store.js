...
import { bootstrapSleepFromHealthKit } from './sleepInference';
...
const useStore = create((set, get) => ({
  ...
  initialize: async () => {
    try {
      const [userData, appData, alarms, wakeEvents, sleepData, sessions, tracking, healthKitAuth, lastSync] = await Promise.all([
  ...
  syncHealthKitData: async () => {
    try {
      ...
      if (healthKitSessions && healthKitSessions.length > 0) {
        ...
        // Otherwise, try to infer from steps
        console.log('[Store] No direct sleep data, attempting step-based inference...');
        const inferenceResult = await bootstrapSleepFromHealthKit();

        if (inferenceResult.success && inferenceResult.sessions.length > 0) {
          console.log('[Store] Successfully inferred sleep from steps:', inferenceResult.sessions.length);
          sessionsToUse = inferenceResult.sessions;
          dataSource = 'healthkit-inferred';
        } else {
          console.log('[Store] No sleep data available from HealthKit');
          return {
            success: false,
            message: inferenceResult.message || 'No data available',
            count: 0,
          };
        }
      }
