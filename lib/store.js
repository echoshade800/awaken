...
const useStore = create((set, get) => ({
  ...
  getSleepSessionsForDebtChart: () => {
    ...
    const dateMap = {};
    sessions
      .filter((s) => s.date >= fourteenDaysAgo && s.date <= today)
      .forEach((s) => {
        if (!dateMap[s.date] || dateMap[s.date].id < s.id) {
          dateMap[s.date] = s;
        }
  ...
  insertDemoSleepData: async () => {
    ...
    const hasDemoData = existingSessions.some((s) => s.source === 'demo');
    const hasRealData = existingSessions.some((s) => s.source !== 'demo');
