...
import {
  fetchAndProcessHealthData,
  shouldRefreshHealthData,
  getHealthDataFromStorage,
} from './healthDataOrchestrator';
...
const useStore = create((set, get) => ({
  ...
  initialize: async () => {
    try {
      ...
      const hasHealthPermission = await checkHealthPermission();
      ...
      set({ isLoading: false, healthPermissionChecked: true });
    }
  },
  ...
  requestHealthPermission: async () => {
    try {
      ...
      set({ hasHealthPermission: granted, healthPermissionChecked: true });
      ...
      console.error('Failed to request health permission:', error);
      set({ hasHealthPermission: false, healthPermissionChecked: true });
      return false;
    }
  },
  ...
  syncHealthData: async () => {
    try {
      const shouldRefresh = await shouldRefreshHealthData();

      if (!shouldRefresh) {
        console.log('[Store] Using cached health data');
        const cachedData = await getHealthDataFromStorage();
        if (cachedData) {
          set({
            sleepNeed: cachedData.sleepNeedMin / 60,
            sleepDebt: cachedData.sleepDebtMin / 60,
            circadianCurve: cachedData.circadianDay,
            healthDataAvailable: cachedData.healthDataAvailable,
          });
        }
        return cachedData;
      }

      console.log('[Store] Fetching fresh health data...');
      const result = await fetchAndProcessHealthData();

      if (result) {
        set({
          sleepNeed: result.sleepNeedMin / 60,
          sleepDebt: result.sleepDebtMin / 60,
          circadianCurve: result.circadianDay,
          sleepSessions: result.sessions,
      ...
      console.error('[Store] Failed to sync health data:', error);
      return null;
    }
  },
  ...
  checkHealthPermissionStatus: async () => {
    try {
      const hasPermission = await checkHealthPermission();
      set({ hasHealthPermission: hasPermission, healthPermissionChecked: true });
      return hasPermission;
    } catch (error) {
      console.error('Failed to check health permission:', error);
      set({ hasHealthPermission: false, healthPermissionChecked: true });
      return false;
    }
  },
