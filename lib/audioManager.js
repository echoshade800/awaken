...
export async function playAlarmRingtone(ringtoneUri = null, options = {}) {
  try {
    ...
    if (ringtoneUri && ringtoneUri.startsWith('http')) {
      console.log('[AudioManager] Loading ringtone from URL:', ringtoneUri);
      try {
        ...
        console.error('[AudioManager] Failed to load custom ringtone, using fallback');
        return await playSystemAlarmSound();
      }
    } else {
      console.log('[AudioManager] No custom ringtone, using system sound');
      return await playSystemAlarmSound();
    }
