...
export async function fetchDailySteps14d() {
  console.log('[HealthKit] fetchDailySteps14d (bridge-driven), Platform:', Platform.OS);
  ...
  return new Promise((resolve) => {
    ...
    AppleHealthKit.isAvailable((err, available) => {
      ...
      AppleHealthKit.initHealthKit(permissions, (initErr) => {
        ...
        // Step 3: Calculate time range (last 14 days)
        const end = new Date();
        ...
        // Step 4: Fetch daily step count samples
        AppleHealthKit.getDailyStepCountSamples(options, (stepErr, results) => {
          if (stepErr) {
            console.error('[HealthKit] Error fetching step data:', stepErr);
            resolve([]);
            return;
          }
          ...
          const formattedData = results.map(item => ({
            ...
            value: item.value || 0, // Number of steps
          }));

          console.log('[HealthKit] fetchDailySteps14d mapped sample:', formattedData.slice(0, 10));
          console.log('[HealthKit] Total days of data:', formattedData.length);
