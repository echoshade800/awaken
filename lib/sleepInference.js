...
export async function inferSleepFromRecentSamples() {
  ...
  try {
    // Step 1: Fetch and sort 24h step samples
    ...
    console.log('[SleepInference] candidate blocks:', blocks.map(b => ({
      startISO: b.startISO,
      endISO: b.endISO,
      durationMin: b.durationMin,
      quietRatio: b.quietRatio,
    })));
    ...
    // Step 5: Choose main sleep block
    let mainBlock = null;

    // Priority 1: Block >= 8 hours (480 min)
    const longBlocks = blocks.filter(b => b.durationMin >= 480);
    if (longBlocks.length > 0) {
      mainBlock = longBlocks[0];
      console.log('[SleepInference] chosen main block: 8+ hour block');
    } else {
      // Priority 2: Score-based selection (>= 4 hours)
      const eligibleBlocks = blocks.filter(b => b.durationMin >= 240);
      if (eligibleBlocks.length > 0) {
        eligibleBlocks.forEach(b => {
          b.score = b.durationMin * b.quietRatio;
        });
        eligibleBlocks.sort((a, b) => b.score - a.score);
        mainBlock = eligibleBlocks[0];
        console.log('[SleepInference] chosen main block: highest score (>=4h)');
      } else {
        // Priority 3: Fallback - longest block regardless of duration
        blocks.sort((a, b) => b.durationMin - a.durationMin);
        mainBlock = blocks[0];
        console.log('[SleepInference] chosen main block: longest available (fallback)');
      }
    }

    console.log('[SleepInference] chosen main block:', {
      startISO: mainBlock.startISO,
      endISO: mainBlock.endISO,
      durationMin: mainBlock.durationMin,
      quietRatio: mainBlock.quietRatio,
    });
    ...
    console.log('[SleepInference] refined bedtime:', {
      bedtime: refinedBedtime,
      waketime: refinedWaketime,
